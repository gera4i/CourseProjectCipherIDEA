...
public class IdeaCipher extends BlockCipher {
    ...
    private int[] subKey;
    public static String subkeyForBinary = "";
    public static String subkeyForUI = "";
    ...
    private static int[] generateSubkeys(byte[] userKey) {
        ...
        // The first four are used in round 2; the ladst four are used in round 3.
        ...
        for (byte b : userKey) {
            sb.append(FileCipher.toPrettyBin(b, 8));
        }
        subkeyForUI += "User key = " + sb.toString() + "\n";
        for (int i = 0; i < key.length; i++) {
            subkeyForUI += String.format("Subkey %d = %s\n", i+1, FileCipher.toPrettyBin(key[i], 16));
        }
